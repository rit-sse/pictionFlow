...
For our purposes, we will have to modify this file to accomodate for ModeKeys.PREDICT,
and we will have to modify main. We will train this model with the Quick, Draw! data.

The Estimator will have to be returned when setting up the server. I believe we can probably
just call Estimator.predict() once we add that mode to the estimator. 

We will have the data to be used for the predication stored in a file to be referenced with a 
flag. the flag is known to python internally as FLAG.predict_data; and it will be passed as 
--predict_data "data/predict_data.". This would mean that we could only make one prediction at a time.

Now, this will be too slow, so we will have to consider multi-threading multiple of these
models at once.
"""
